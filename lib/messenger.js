...
//
// Login calls are executed differently (see login.js)
...
class Messenger {
    ...
    // callback(err) does not get any data
    sendAttachment(recipient, recipientId, filePath, callback) {
      const recipientUrl = `${this.baseUrl  }/t/${  recipient}`; // Your recipient;
      const utcTimestamp = new Date().getTime();
      const localTime = new Date().toLocaleTimeString().replace(/\s+/g, '').toLowerCase();
      const messageId = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);

      // header is identical in both the upload endpoint and message-send endpoint
      const headers = {
        'origin': 'https://www.messenger.com',
        'accept-encoding': 'gzip, deflate',
        'x-msgr-region': 'ATN',
      ...
      request.post("https://upload.messenger.com/ajax/mercury/upload.php", {
          headers: headers,
          formData: {
              // the ones commented out were parameters I wasn't sure whether
              // they were needed but are confirmed unneeded now.
              '__a': '1',
              // '__be': '0',
          ...
          // if Facebook denies the file
          if (bodyJson.error) {
              // error details: bodyJson.errorSummary, bodyJson.errorDescription
              console.log("Attachment denied by Facebook");
              return;
          }

          let formData = {
            // everything's identical to the formData in sendMessage except:
            // 1. no "body"
            // 2. "has_attachment" set to true
            // 3. no "html_body"
            // 4. the id to the attachment is added in the if/else block below
            'action_type': 'ma-type:user-generated-message',
            'author': `fbid:${this.userId}`,
            'timestamp': utcTimestamp,
            'timestamp_absolute': 'Today',
          ...
          let metadata = bodyJson.payload.metadata[0];
          if (metadata.image_id) {
              formData['image_ids[0]'] = metadata.image_id;
          } else if (metadata.video_id) {
              formData['video_ids[0]'] = metadata.video_id;
          } else if (metadata.audio_id) {
              formData['audio_ids[0]'] = metadata.audio_id;
          } else {
    ...
    // Send an attachment in a Group thread
    //   recipientId : Facebook numeric id of recipient. Can be a person or a thread
    //   filePath : path of the file
    // callback(err) does not get any data
    sendGroupAttachment(recipientId, filePath, callback) {
      const recipientUrl = `${this.baseUrl  }/t/${  recipientId}`; // Your recipient;
      const utcTimestamp = new Date().getTime();
      const localTime = new Date().toLocaleTimeString().replace(/\s+/g, '').toLowerCase();
      const messageId = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
      ...
      request.post("https://upload.messenger.com/ajax/mercury/upload.php", {
          ...
          let formData = {
            ...
            // 1. no "body"
            // 2. "has_attachment" set to true
            // 3. the id to the attachment is added in the if/else block below
            'action_type': 'ma-type:user-generated-message',
            'author': `fbid:${this.userId}`,
            'timestamp': utcTimestamp,
            'source': 'source:messenger:web',
    ...
    getThreads(callback) {
        ...
        request.post(options, (err, response, body) => {
            ...
            try {
                ...
                if (results && results.o0 && results.o0.data) {
