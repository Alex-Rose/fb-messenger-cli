...
class Login {
    ...
    execute(callback, options) {
        ...
        while ((!this.user || !this.pass) && --attemptsRemaining >= 0) {
            if (attemptsRemaining < maxRetry){
                console.log(`Please provide email/password. ${attemptsRemaining + 1} attempts remaining`);
            }
            this.user = readlineSync.question('Email: ');
            this.pass = readlineSync.question('Password: ', { hideEchoBack: true });
        }

        if (!this.user || !this.pass && attemptsRemaining){
            callback(new Error('Email and password cannot be empty'));
            return;
        }
